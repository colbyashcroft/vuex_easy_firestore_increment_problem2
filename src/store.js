...
books = books.map((book) => {return {title: book.title, author: book.author}});

const myModule = {
  firestorePath: 'booksList',
  ...
  getters: {
    currentBooks(state) {
      ...
      if(state.data && Object.keys(state.data).length > 0){
        ...
        console.log('Object.values(state.data)', Object.values(state.data));
        const objectOfInterest = Object.values(state.data)[0]
        const booksArray = Object.values(objectOfInterest.books);
        console.log('booksArray', booksArray);
        returnValue = booksArray.sort((a,b) => a.order - b.order);
      }
      return returnValue;
    }
  },
  ...
  actions: {
    ...
    resetBooksList({dispatch, state}){
      const booksObj = {
        abcd: books[0],
        efgh: books[1],
        ijkl: books[2]
      }
      booksObj.abcd.id = 'abcd'
      booksObj.abcd.order = 0;
      booksObj.efgh.id = 'efgh'
      booksObj.efgh.order = 1;
      booksObj.ijkl.id = 'ijkl'
      booksObj.ijkl.order = 2;
      dispatch('patch', {id: Object.keys(state.data)[0], books: {}})
      dispatch('batchSync');
      dispatch('patch', {id: Object.keys(state.data)[0], books: booksObj})
    },
    addBookOrder1({dispatch, getters, state}){
      const {currentBooks} = getters
      const newBook = books[Math.floor(Math.random()*books.length)]
      const update = {id: Object.keys(state.data)[0]}
      const newBookId = getters['dbRef'].doc().id
      update.books = {}
      update.books[newBookId] = newBook
      update.books[newBookId].id = newBookId
      update.books[newBookId].order = 1

      currentBooks.forEach((book) => {
        if(book.order >= 1){
          update.books[book.id] ={order: increment(1)}
        }
      })
      dispatch('patch', update)
      dispatch('batchSync');
    },
    editTitleOrder0({dispatch, state, getters}){
      const {currentBooks} = getters
      const bookOfInterest = currentBooks.find((value) => value.order === 0)
      const newBook = books[Math.floor(Math.random()*books.length)]
      const update = {id: Object.keys(state.data)[0]}
      update.books = {}
      update.books[bookOfInterest.id] = {}
      update.books[bookOfInterest.id].title = newBook.title;
      dispatch('patch', update)
    }
  },
}
